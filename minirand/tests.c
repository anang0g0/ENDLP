...
static struct chacha20_testvector chacha20_testvectors[5] = {
...
int eringi(void)
{
    ...
    uint8_t keystreac[512];
    uint8_t poly1305_tag[16];

    //for(int j=0;j<512;j++)
    //keystream[j]=j;
    //chacha20_testvectors[4].resulting_keystream[0]=0xff;
    //for(i=0;i<32;i++)
    //chacha20_testvectors[4].key[i]=i;//rand()%256;
   /*
    int j=0;
    for(i=0;i<5;i++){
        for(j=0;j<32;j++)
    chacha20_testvectors[i].key[j]=1;
    }
    */
    /* test chacha20 */
    for (i = 0;
        ...
        chacha_encrypt_bytes(&ctx, keystream, keystreac, 512);
        assert(memcmp(keystreac, chacha20_testvectors[i].resulting_keystream,
                   chacha20_testvectors[i].keystream_check_size) == 0);
    for(int j=0;j<chacha20_testvectors[i].keystream_check_size;j++)
        printf("%d %d, %u, %u \n", i, j, keystreac[j], chacha20_testvectors[i].resulting_keystream[j]);
    }
//exit(1);


    static struct chacha20_testvector aaa = {
        ...
        chacha_ivsetup(&ctx, aaa.nonce, NULL);
        memset(keystream, 0, 512);
        //keystream[257]=0xff;
        for(int j=0;j<10;j++){
        chacha_keysetup(&ctx, aaa.key, 256);
        chacha_encrypt_bytes(&ctx, keystream, keystreac, 512);
    for(i=0;i<aaa.keystream_check_size;i++)
        printf("u%d %u, %u\n", i, keystream[i],keystreac[i]);

        memset(keystream, 0, 512);
        memcpy(keystream,keystreac,sizeof(keystream));
        //assert(memcmp(keystream, aaa.resulting_keystream,
        //           aaa.keystream_check_size) == 0);
        memset(keystreac, 0, 512);
        }
    //exit(1);
