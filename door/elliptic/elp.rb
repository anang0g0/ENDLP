...
#CRV.name="eccp256";
@CRV_a=-3;
@CRV_b=41058363725152142129326129780047268409114441015993725554835256314039467401291;
@CRV_p=115792089210356248762697446949407573530086143415290314195533631308867097853951;
@CRV_G_x=48439561293906451759052585252797914202762949526041747995844080717082404635286;
@CRV_G_y=36134250956749795798585127919587881956611106672985015071877198253568414405109;
...
class Alice
    def initialize
        ...
        @c = randInt
        @d = randInt
        @A = randMatrix.map{|x| CRV.G * x}
    end

    def public_keys
        @X = randMatrix.map{|x| IntegerModRing(P).new(x)}
        @Y = @X**@a * @A*@c * @X**-@a
        @Z = @X**@b * @A*(@c * @d) * @X**-@b
        [@X, @Y, @Z]
    end
