...
def randMatrix
    Matrix.build(2){randInt} #Matrix.build(2){IntegerModRing(P).new(randInt)}
end
...
class Alice
    ...
    def public_keys
        @X = randMatrix.map{|x| IntegerModRing(P).new(x)} #Matrix[[randInt,randInt],[randInt,randInt]].map{|x| IntegerModRing(P).new(x)}
        @Y = @X**@a * @A*@c * @X**-@a
        @Z = @X**@b * @A*(@c * @d) * @X**-@b
        [@X, @Y, @Z]
    end
