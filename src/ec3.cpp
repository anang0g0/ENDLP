...
void init_curve(int n)
{
	...
	{
		/*
			CRV.a = to_ZZ("6864797660130609714981900799081393217269435300143305409394463459185543183397656052122559640661454554977296311391480858037121987999716643812574028291115057148");
			CRV.b = to_ZZ("1093849038073734274511112390766805569936207598951683748994586394495953116150735016013708737573759623248592132296706313309438452531591012912142327488478985984");
			CRV.p = to_ZZ("6864797660130609714981900799081393217269435300143305409394463459185543183397656052122559640661454554977296311391480858037121987999716643812574028291115057151");
			CRV.n = to_ZZ("6864797660130609714981900799081393217269435300143305409394463459185543183397655394245057746333217197532963996371363321113864768612440380340372808892707005449");
			CRV.G.x = to_ZZ("2661740802050217063228768716723360960729859168756973147706671368418802944996427808491545080627771902352094241225065558662157113545570916814161637315895999846");
			CRV.G.y = to_ZZ("3757180025770020463545507224491183603594455134769762486694567779615544477440556316691234405012945539562144444537289428522585666729196580810124344277578376784");
		*/

		CRV.a = to_ZZ("696485084002814808960886295766581041443945449814652352298627881181752850079587199048987616200806976787542938844075125535760582937924063814674890735664317823");
		CRV.b = to_ZZ("464323389335209872640590863844387360962630299876434901532418587454501900053058132699325077467204651191695292562716750357173721958616042543116593823776211882");
		CRV.n = to_ZZ("3432398830065304857490950399540696608634717650071652704697231729592771591712973187682510366763670709017309470519095890345315645302918460368693682062774007869");
		CRV.p = to_ZZ("3432398830065304857490950399540696608634717650071652704697231729592771591712969482335654772645417154745789192506044585705806344804869197726005428842625522283");
		CRV.G.x = to_ZZ("0");
		CRV.G.y = to_ZZ("1328942039729182261230561285102907024478976990392216105229481040028285177125590812153133647247446498108102527828467093007922119058723057613132209889245369858");
	}
	if (n == 224)
	{
...
*/
		CRV.a = to_ZZ("54938263600476024118477719419064585563422145858861416387656371338832307054606");
		CRV.b = to_ZZ("36625509066984016078985146279376390375614763905907610925104247559221538036404");
		CRV.n = to_ZZ("115792089237316195423570985008687919880550243486210555152433083683760878173979");
		CRV.p = to_ZZ("115792089237316195423570985008687919879869678752368678225506334468897341715723");
		CRV.G.x = to_ZZ("0");
		CRV.G.y = to_ZZ("112576631489968869256357416093040033590807003528314110625659765475993848880984");
	}
	...
	if (n == 2566)
	{
		...
		//  CRV.G.z = 1;
		CRV.a = to_ZZ("99565108300935097535931366500189844917575237742803292519459746379507877342043");
		CRV.b = to_ZZ("27779375788184666549430582663897260650531786721334165917073856229891750424625");
		CRV.n = to_ZZ("115792089237316195423570985008687907882874550587762210360771174854476966987211");
		CRV.p = to_ZZ("115792089237316195423570985008687907883555115321604087287697924069340503410211");
		CRV.G.x = to_ZZ("0");
		CRV.G.y = to_ZZ("99200793273574428735126656404077644938682984790297937725393004948021375699450");
	}
...
ten elp3(ZZ k)
{
	...
	if (k > 0)
	{
		for (i = 1; i < j; i++)
		{
			if (Pubkey.x != ll[ki[i]].x || Pubkey.y != ll[ki[i]].y || Pubkey.z != ll[ki[i]].z)
			{

				if (Pubkey.y * Pubkey.y % CRV.p == (Pubkey.x * Pubkey.x * Pubkey.x + CRV.a * Pubkey.x * (Pubkey.z * Pubkey.z * Pubkey.z * Pubkey.z) + CRV.b * (Pubkey.z * Pubkey.z * Pubkey.z * Pubkey.z * Pubkey.z * Pubkey.z)) % CRV.p)
				{
					// cout<<"doko2\n";
					if (ll[ki[i]].y * ll[ki[i]].y % CRV.p == (ll[ki[i]].x * ll[ki[i]].x * ll[ki[i]].x + CRV.a * ll[ki[i]].x * (ll[ki[i]].z * ll[ki[i]].z * ll[ki[i]].z * ll[ki[i]].z) + CRV.b * (ll[ki[i]].z * ll[ki[i]].z * ll[ki[i]].z * ll[ki[i]].z * ll[ki[i]].z * ll[ki[i]].z)) % CRV.p)
					{
						...
						Pubkey.y = P.y;
						Pubkey.z = P.z;
						if (Pubkey.x == 0 && Pubkey.y == 0 && Pubkey.z == 0)
						{
							//	if(Pubkey_z==0)
			...
			if (Pubkey.x == ll[ki[i]].x && Pubkey.y == ll[ki[i]].y && Pubkey.z == ll[ki[i]].z)
			{
				...
				// print"ecole\n"
				jdbl(Pubkey);
				cout << "edb2(" << Pubkey.x << Pubkey.y << Pubkey.z << ")\n";
				exit(1);
			}
			// print"inforloop",i,"";
		}
...
po t2p(ten x)
{
	...
	z.x = x.x*inv(x.z,CRV.p);
	z.y = x.y*inv(x.y,CRV.p);
	if(x.z>0){
	cout << "heee\n";
	exit(1);
	}
...
{
	kem n = {0};
	n.u = (inv2(it(b.u), NP) * a.v - inv2(it(b.u), NP) * a.v) % NP;
	n.v = be(a.v) * inv2(be(a.v), NP) % NP;
	// n.u %= 23;
...
esem sabun(ZZ a, ZZ b, esem X, esem Y)
{
	...
	B = Emul(Emul(Qpow(b, X), B), invs(Qpow(b, X)));
...
void ehw()
{
	...
	a = ZZ(rand() % 41);
	// to_ZZ("3");
	b = ZZ(rand() % 41); // to_ZZ("4");
	c = ZZ(rand() % 41); // to_ZZ("789");
	r = ZZ(rand() % 41); // to_ZZ("517");
	s = ZZ(rand() % 41); // to_ZZ("17");
	// A.u = CRV.G;
	// A.v = to_ZZ("2");
	A = vom();
	// X.u = Qmlt(CRV.G, a);
...
void epp()
{
	...
	esem E = Qadd(Qadd(Qadd(Qpow(z, A), Qpow(x, A)), Qpow(t, B)), Qadd(Qpow(y, C), Qpow(w, C)));
...
for(int k=1;k<1;k++){
tmp=Qpow(ZZ(i),A);
//tmp=esemi(esemi(Qpow(ZZ(i), A),Qpow(ZZ(k),B)), Qpow(ZZ(j), C));
if(is_eq(tmp,A)==1){
printf("i=%d k=%d j=%d\n",i,k,j);
...
ve einv(ve e)
{
	e.e[0] = (Qmlt(e.e[0], e.v[0]));
	e.e[1] = (Qmlt(e.e[1], e.v[1]));
	e.v[0] = inv(e.v[0], CRV.n);
	e.v[1] = inv(e.v[1], CRV.n);
...
ve tdk(ve a, ve b, ve c)
{
	return Emul(Emul(a, b), einv(c));
}
...
kem kemi2(kem a, kem b)
{
	kem n = {0};
	n.u = ((be(a.u) * b.v) % NP + b.u) % NP;
	n.v = ((a.v) * b.v) % NP;
	n.u %= NP;
